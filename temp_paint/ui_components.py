...
# --- FRAGMENTS ---
def cb_sidebar_color_picker():
    """Callback to synchronize sidebar color picker changes to master state immediately."""
    if "sidebar_paint_cp" in st.session_state:
        new_color = st.session_state["sidebar_paint_cp"]
        st.session_state["picked_color"] = new_color
        
        # Keep text box in sync
        st.session_state["sidebar_color_hex"] = new_color
        st.session_state["search_color_results"] = None
        
        # Dynamically update the color of the latest active layer
        if st.session_state.get("masks"):
            st.session_state["masks"][-1]["color"] = new_color
            st.session_state["comp_cache_state"] = None
            st.session_state["comp_cache_len"] = 0
            st.session_state["render_id"] += 1

def cb_frag_color_picker():
    """Callback to synchronize pending selection color picker changes to master state immediately."""
    if "frag_pending_color" in st.session_state:
        st.session_state["picked_color"] = st.session_state["frag_pending_color"]

def resolve_color_input(input_val: str) -> tuple[bool, any, str]:
    """
    Validates and resolves color input (Hex, Berger shade code, or Berger shade name).
    Returns (success, hex_color_or_list, message).
    """
    import re
    import requests
    ...
    if not clean_val:
        return False, None, "Please enter a color value."
        
    # 1. Check if it matches Berger Paints shade code pattern: e.g. 1A0316, 6P0172, 1D2819, etc.
    # Check this FIRST so that codes like 1A0316 are not misidentified as raw hex colors.
    berger_match = re.match(r"^#?([0-9][a-zA-Z][0-9]{4})$", clean_val)
    if berger_match:
        code = berger_match.group(1).upper()
        url = f"https://www.bergerpaints.com/api/search/{code}"
        try:
            headers = {
            ...
            if response.status_code == 200:
                ...
                for shade in shades:
                    if shade.get("field_shadecode", "").upper() == code:
                        hex_code = shade.get("field_hexcode")
                        name = shade.get("name", "Unknown")
                        if hex_code:
                            if not hex_code.startswith("#"):
                                hex_code = "#" + hex_code
                            return True, hex_code, f"Resolved Berger shade: {name} ({code}) -> {hex_code}"
        except Exception:
            pass

    # 2. Check if it is a valid hex color code
    hex_check = clean_val.lstrip("#")
    if len(hex_check) in [3, 6, 8] and all(c in "0123456789abcdefABCDEF" for c in hex_check):
        resolved_hex = "#" + hex_check
        return True, resolved_hex, f"Valid hex color: {resolved_hex}"
        
    # 3. Treat as search query for Berger Paints API (Shade name)
    url = f"https://www.bergerpaints.com/api/search/{clean_val}"
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        }
        ...
        if response.status_code == 200:
            ...
            # De-duplicate shades by field_shadecode
            unique_shades = {}
            for shade in shades:
                code = shade.get("field_shadecode")
                hex_code = shade.get("field_hexcode")
                name = shade.get("name")
                if code and hex_code and name:
                    if not hex_code.startswith("#"):
                        hex_code = "#" + hex_code
                    unique_shades[code.upper()] = {
                        "name": name,
                        "code": code.upper(),
                        "hex": hex_code
                    }
                    
            shade_list = list(unique_shades.values())
            if len(shade_list) == 1:
                single_shade = shade_list[0]
                return True, single_shade["hex"], f"Resolved Berger shade: {single_shade['name']} ({single_shade['code']}) -> {single_shade['hex']}"
            elif len(shade_list) > 1:
                return True, shade_list, f"Found {len(shade_list)} matching Berger shades."
    except Exception:
        pass
        
    return False, None, f"Invalid color: '{input_val}'. Enter a hex code (e.g. #FF4B4B) or Berger shade name/code (e.g. Shibuya Pink or 6P1417)."

def cb_sidebar_hex_input():
    """Callback to handle pasted hex color code or Berger shade code values directly."""
    if "sidebar_color_hex" in st.session_state:
        raw_input = st.session_state["sidebar_color_hex"].strip()
        if not raw_input:
            return
            
        success, res, msg = resolve_color_input(raw_input)
        if success:
            st.session_state["color_input_error"] = None
            if isinstance(res, str):
                hex_color = res
                # Only update the active paint color for the NEXT tap.
                # Never mutate already-applied layers — each layer keeps its own color.
                st.session_state["picked_color"] = hex_color
                st.session_state["sidebar_paint_cp"] = hex_color
                st.session_state["sidebar_color_hex"] = hex_color
                st.session_state["search_color_results"] = None
            else:
                st.session_state["search_color_results"] = res
        else:
            st.session_state["color_input_error"] = msg
            st.session_state["search_color_results"] = None
...
def sidebar_paint_fragment():
    ...
    # Sanitize picked_color to prevent StreamlitAPIException on load if it contains invalid hex (e.g. from previous errors)
    if (not isinstance(active_color, str) or 
        not active_color.startswith("#") or 
        len(active_color) not in [4, 7, 9] or 
        not all(c in "0123456789abcdefABCDEF" for c in active_color[1:])):
        active_color = "#8FBC8F"
        st.session_state["picked_color"] = "#8FBC8F"
        st.session_state["sidebar_paint_cp"] = "#8FBC8F"
        st.session_state["sidebar_color_hex"] = "#8FBC8F"
    ...
    with st.container(border=True):
        ...
        with col_p:
            new_color = st.color_picker(
                ...
                on_change=cb_sidebar_color_picker
            )
        with col_t:
             st.markdown(f"""
    ...
    # Hex paste text field
    st.text_input(
        "Or paste Color Hex Code:",
        value=st.session_state.get("sidebar_color_hex", active_color),
        key="sidebar_color_hex",
        on_change=cb_sidebar_hex_input,
        help="Paste a color hex code (e.g. #FF4B4B) or Berger shade code (e.g. 6P1417) and press Enter."
    )
    
    error_msg = st.session_state.get("color_input_error")
    if error_msg:
        st.error(error_msg)

    # Show shade search results when multiple matches found
    search_results = st.session_state.get("search_color_results")
    if search_results and isinstance(search_results, list):
        st.caption(f"Found {len(search_results)} matching shades — click to apply:")
        cols_per_row = 3
        for i in range(0, len(search_results), cols_per_row):
            row_shades = search_results[i:i + cols_per_row]
            cols = st.columns(cols_per_row)
            for col, shade in zip(cols, row_shades):
                with col:
                    hex_val = shade["hex"]
                    name = shade["name"]
                    code = shade["code"]
                    st.markdown(
                        f'<div style="width:100%;height:28px;background:{hex_val};border-radius:6px;border:1px solid #ccc;margin-bottom:4px;"></div>',
                        unsafe_allow_html=True
                    )
                    if st.button(f"{code}", key=f"shade_pick_{code}_{i}", use_container_width=True, help=f"{name} — {hex_val}"):
                        st.session_state["picked_color"] = hex_val
                        st.session_state["sidebar_paint_cp"] = hex_val
                        st.session_state["sidebar_color_hex"] = hex_val
                        st.session_state["search_color_results"] = None
                        st.session_state["color_input_error"] = None
                        if st.session_state.get("masks"):
                            st.session_state["masks"][-1]["color"] = hex_val
                            st.session_state["comp_cache_state"] = None
                            st.session_state["comp_cache_len"] = 0
                            st.session_state["render_id"] += 1
                        st.rerun()

    # Automatically submit hex color on typing/pasting without pressing Enter
    import streamlit.components.v1 as components
    components.html("""
    <script>
    const doc = window.parent.document;
    function setupAutoEnter() {
        // Robust label and input identification using HTML relationship
        const labels = Array.from(doc.querySelectorAll('label'));
        const hexLabel = labels.find(l => l.textContent && l.textContent.includes("paste Color Hex Code"));
        const hexInput = hexLabel ? doc.getElementById(hexLabel.getAttribute('for')) : null;
        
        if (hexInput && !hexInput.dataset.autoEnterAttached) {
            hexInput.dataset.autoEnterAttached = "true";
            hexInput.addEventListener('input', (e) => {
                ...
                // Matches standard 3-character or 6-character hex colors
                if (cleanVal.match(/^#[0-9A-Fa-f]{6}$/) || cleanVal.match(/^#[0-9A-Fa-f]{3}$/)) {
                    // Update input value through native HTML setter to bypass React state interception
                    try {
                        const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
                        nativeSetter.call(hexInput, cleanVal);
                    } catch(err) {
                        const tracker = hexInput._valueTracker;
                        if (tracker) { tracker.setValue(cleanVal); }
                    }
                    
                    // Dispatch change events to bubble up to React
                    hexInput.dispatchEvent(new Event('input', { bubbles: true }));
                    hexInput.dispatchEvent(new Event('change', { bubbles: true }));
                    
                    // Trigger enter to force Streamlit update
                    ...
                    hexInput.dispatchEvent(enterEvent);
    ...
    setInterval(setupAutoEnter, 500);
    </script>
    """, height=0, width=0)
...
def render_visualizer_canvas_fragment_v11(display_width, start_x, start_y, view_w, view_h, scale_factor, h, w, drawing_mode):
    ...
    from paint_utils.image_processing import safe_composite_pipeline, apply_camera_filter
    painted_img = safe_composite_pipeline(base_image, st.session_state["masks"])
    
    filter_name = st.session_state.get("camera_filter", "None")
    painted_img = apply_camera_filter(painted_img, filter_name)
    base_image_filtered = apply_camera_filter(base_image, filter_name)
    ...
    if show_comp:
        display_img = base_image_filtered
    else:
    ...
    canvas_result = st_canvas(
        ...
        key=f"canvas_main_{st.session_state.get('canvas_id', 0)}_{st.session_state.get('render_id', 0)}", 
        ...
        if objects:
                    ...
                    if obj["type"] == "path":
                        ...
                        if path_data:
                            ...
                            if mask.any():
                                ...
                                op = "Subtract" if "Eraser" in tool_mode else st.session_state.get("selection_op", "Add")
                                selected_idx = st.session_state.get("selected_layer_idx")
                                
                                merged = False
                                
                                if selected_idx is not None and selected_idx < len(st.session_state["masks"]):
                                    # Target the selected layer directly!
                                    target_layer = st.session_state["masks"][selected_idx]
                                    target_mask = target_layer['mask']
                                    if sparse.issparse(target_mask):
                                        target_mask = target_mask.toarray()
                                        
                                    if op == "Add":
                                        target_layer['mask'] = sparse.csc_matrix(target_mask | mask)
                                        print(f"DEBUG: Brushed onto selected Layer {selected_idx+1}")
                                    else:
                                        target_layer['mask'] = sparse.csc_matrix(target_mask & ~mask)
                                        print(f"DEBUG: Erased from selected Layer {selected_idx+1}")
                                    ...
                                    # Existing fallback behavior: merge with last brush layer or create new layer
                                    if st.session_state["masks"]:
                                        ...
                                        if is_brush_layer and is_same_color and is_same_op:
                                            ...
                                            if op == "Add":
                                                last_layer['mask'] = sparse.csc_matrix(target_mask | mask)
                                            else:
                                                # Erase from the last brush layer
                                                last_layer['mask'] = sparse.csc_matrix(target_mask & ~mask)
                                            ...
                                            merged = True
                                            print(f"DEBUG: Merged brush stroke into last Layer")
            ...
            if st.session_state.get("pending_selection") is not None:
                ...
                with st.container(border=True):
                    ...
                    with b_col2: 
                        new_chosen = st.color_picker(
                            "Color", 
                            st.session_state.get("picked_color", "#8FBC8F"), 
                            label_visibility="collapsed", 
                            key="frag_pending_color",
                            on_change=cb_frag_color_picker
                        )
...
def render_sidebar(sam, device_str):
    ...
    with st.sidebar:
        ...
        if st.session_state.get("selected_layer_idx") is not None:
            ...
            if idx < len(st.session_state["masks"]):
                layer = st.session_state["masks"][idx]
                changed = False
                for prop, state_key in [
                    ('color', 'picked_color'),
                    ('softness', 'selection_softness'),
                    ('refinement', 'selection_refinement'),
                    ('fidelity', 'selection_fidelity'),
                    ('finish', 'selection_finish'),
                    ('opacity', 'selection_highlight_opacity'),
                    ('exposure', 'selection_exposure'),
                    ('shadows', 'selection_shadows'),
                ]:
                    if state_key in st.session_state and st.session_state[state_key] != layer.get(prop):
                        layer[prop] = st.session_state[state_key]
                        changed = True
                if changed:
        ...
        if st.session_state.get("image") is not None:
            if st.session_state["masks"]:
                ...
                if st.button(_dl_label, use_container_width=True, type="primary"):
                    ...
                    try:
                        ...
                        filter_name = st.session_state.get("camera_filter", "None")
                        dl_comp = apply_camera_filter(dl_comp, filter_name)
            ...
            has_pending = st.session_state.get("pending_selection") is not None
            selected_idx = st.session_state.get("selected_layer_idx")
            has_selected = selected_idx is not None and selected_idx < len(st.session_state["masks"])
            
            if has_pending or has_selected:
                exp_label = f"✏️ Edit Layer {selected_idx+1} Settings" if has_selected else "✨ Selection Layer Tuning"
                with st.expander(exp_label, expanded=True):
                    ...
                    # Color Fidelity
                    st.slider("Color Fidelity (Exact RGB)", 0.0, 1.0, 0.35, key="selection_fidelity", help="0.35 (default) = photorealistic with lighting/shadows preserved. 1.0 = flat paint-chip look. 0.0 = full realism (very dark colors follow original shadows exactly).")
                    
                    # Exposure and Shadows
                    col1, col2 = st.columns(2)
                    with col1:
                        st.slider("Exposure", -1.0, 1.0, 0.0, step=0.1, key="selection_exposure", help="Adjust brightness of this layer.")
                    with col2:
                        st.slider("Shadows", -1.0, 1.0, 0.0, step=0.1, key="selection_shadows", help="Lighten or darken shadows in this layer.")
            ...
            def _on_cache_invalidate():
                """Invalidate compositor cache so canvas re-renders in new mode."""
                st.session_state["comp_cache_state"] = None
                st.session_state["comp_cache_len"] = 0
                st.session_state["render_id"] += 1

            st.selectbox("Camera Filter", ["None", "Warm", "Cool", "Cinematic", "Vintage"], key="camera_filter", on_change=_on_cache_invalidate)

            ai_prompt = st.text_input("AI Quick Commands", placeholder="e.g. 'convert to grayscale'")
            if ai_prompt:
                lower_p = ai_prompt.lower()
                if "gray" in lower_p or "grey" in lower_p or "black" in lower_p:
                    if not st.session_state.get("grayscale_mode", False):
                        st.session_state["grayscale_mode"] = True
                        _on_cache_invalidate()
                        st.toast("✅ Grayscale Mode Activated via AI", icon="🤖")
                elif "color" in lower_p or "normal" in lower_p:
                    if st.session_state.get("grayscale_mode", False):
                        st.session_state["grayscale_mode"] = False
                        _on_cache_invalidate()
                        st.toast("✅ Color Mode Activated via AI", icon="🤖")

            st.toggle(
                "🎨 Grayscale Preview Mode",
                key="grayscale_mode",
                on_change=_on_cache_invalidate,
                help=(
            ...
            st.toggle(
                "🖌️ Flat Color Mode (Solid Paint)",
                key="flat_color_mode",
                on_change=_on_cache_invalidate,
                help="Enable to apply flat, solid paint matching the picker exactly without shadows or highlights."
            )
            ...
            if st.session_state["masks"] or st.session_state.get("masks_redo"):
                ...
                for i in range(len(st.session_state["masks"]) - 1, -1, -1):
                    ...
                    with st.container(border=True):
                        try: r1, r2, r3 = st.columns([0.45, 0.40, 0.15], vertical_alignment="center")
                        except: r1, r2, r3 = st.columns([0.45, 0.40, 0.15])
                        cur_c = mask_data.get('color', '#FFFFFF')
                        with r1:
                            is_selected = (st.session_state.get("selected_layer_idx") == i)
                            btn_label = f"🎯 Layer {i+1}" if is_selected else f"Layer {i+1}"
                            if st.button(btn_label, key=f"select_layer_{i}", use_container_width=True, type="primary" if is_selected else "secondary"):
                                if is_selected:
                                    st.session_state["selected_layer_idx"] = None
                                else:
                                    st.session_state["selected_layer_idx"] = i
                                    # Pre-populate session state values from the selected layer
                                    st.session_state["picked_color"] = mask_data.get('color', '#8FBC8F')
                                    st.session_state["selection_softness"] = mask_data.get('softness', 0)
                                    st.session_state["selection_refinement"] = mask_data.get('refinement', 0)
                                    st.session_state["selection_fidelity"] = mask_data.get('fidelity', 0.35)
                                    st.session_state["selection_finish"] = mask_data.get('finish', 'Standard')
                                    st.session_state["selection_highlight_opacity"] = mask_data.get('opacity', 1.0)
                                    st.session_state["selection_exposure"] = mask_data.get('exposure', 0.0)
                                    st.session_state["selection_shadows"] = mask_data.get('shadows', 0.0)
                                    st.session_state["pending_selection"] = None
                                st.session_state["comp_cache_state"] = None
                                safe_rerun()
                        with r2:
                            st.markdown(f"""
                                <div style='display:flex; align-items:center; gap:8px;'>
                                    <div style='width:24px;min-width:24px;height:24px;background:{cur_c};border-radius:4px;border:1px solid #ddd;'></div>
                                    <span style='font-family:monospace;font-size:0.85em;color:#119277;'>{cur_c}</span>
                                </div>
                            """, unsafe_allow_html=True)
                        with r3: st.button("🗑️", key=f"sidebar_del_{i}", on_click=cb_delete_layer, args=(i,))
            else: 
