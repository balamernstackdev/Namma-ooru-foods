        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 md:gap-8">
          {concerns.map((concern) => (
            <Link
              key={concern.name}
              href={concern.href}
              className="group flex flex-col items-center gap-6 p-6 md:p-10 rounded-xl md:rounded-[3rem] bg-slate-50 border border-transparent transition-all hover:bg-white hover:border-slate-100 hover:shadow-premium hover:-translate-y-4"
            >
              <div className={`h-24 w-24 md:h-36 md:w-36 rounded-full ${concern.color} flex items-center justify-center transition-transform duration-700 group-hover:scale-110 shadow-inner p-6 md:p-10`}>
                <concern.icon className="w-full h-full" strokeWidth={1} />
              </div>
              <span className="text-[10px] md:text-[13px] font-black uppercase tracking-[0.3em] text-emerald-950/80 group-hover:text-amber-600 transition-colors whitespace-nowrap text-center">
                {concern.name}
              </span>
            </Link>
          ))}
