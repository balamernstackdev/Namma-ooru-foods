...
const Navbar = () => {
  ...
  const categories = [
    'Grains & Pulses', 'Organic Oils', 'Authentic Spices', 
    'Dairy Products', 'Traditional Snacks', 'Local Sweets',
    'Cold Pressed', 'Pickles & Pastes'
  ];

  return (
    <nav className="sticky top-0 left-0 right-0 z-50 w-full flex flex-col bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm mb-0">
      {/* PRIMARY TIER */}
      <div className="w-full">
        <div className="standard-container mx-auto h-16 md:h-24 flex items-center justify-between gap-4 md:gap-24 py-1">
          {/* LOGO: Anchored Left */}
      ...
      <div className="hidden md:flex w-full bg-slate-50/50 backdrop-blur-sm border-t border-slate-100 h-12 items-center relative">
                  ...
                  <div className="flex flex-col gap-1 px-2">
                    {categories.map((cat) => (
                      <Link 
                        ...
                        className="px-6 py-3 rounded-xl hover:bg-emerald-50 text-[13px] font-bold text-emerald-950 flex items-center justify-between group transition-all"
                      >
                        {cat}
                        <ChevronRight className="h-4 w-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-amber-500" strokeWidth={3} />
                      </Link>
                    ))}
                  </div>
                </div>
      ...
      {isMenuOpen && (
        ...
        >
          ...
          <div className="px-6 py-10 flex flex-col gap-10">
            ...
            {/* Heritage Section: Sub-Card Style */}
            <div className="mt-6">
