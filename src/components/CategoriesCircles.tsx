      ...
      <div className="standard-container flex flex-col gap-8 md:gap-10 lg:gap-12">
        ...
        <div className="relative group">
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-20 w-20 rounded-full bg-white shadow-premium flex items-center justify-center border border-slate-100 text-slate-400 hover:text-emerald-700 opacity-0 group-hover:opacity-100 transition-all disabled:opacity-0 hidden md:flex"
            style={{ transform: 'translate(-50%, -50%)' }}
          >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
          </button>

          <div id="categories-slider" className="flex overflow-x-auto gap-14 md:gap-32 justify-start items-start pt-2 pb-10 snap-x relative no-scrollbar px-6 md:px-24">
            ...
            {categories.map((category) => (
              <Link key={category.name} href={category.href} className="group flex flex-col items-center justify-start transition-all hover:-translate-y-4 snap-center shrink-0">
                <div className="relative w-36 h-36 md:w-80 md:h-80 flex-shrink-0 overflow-hidden rounded-full border-4 md:border-8 border-slate-50 bg-white transition-all group-hover:border-amber-400 group-hover:shadow-premium shadow-md">
                  <Image
                    src={category.image}
                    alt={category.name}
                    fill
                ...
                <span className="text-[11px] md:text-[14px] font-black uppercase tracking-[0.3em] text-slate-500 group-hover:text-emerald-950 transition-colors inline-block mt-8">
                  {category.name}
                </span>
              </Link>
            ))}
