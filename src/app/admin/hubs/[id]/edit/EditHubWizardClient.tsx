...
const parsePermissions = (settingsJson: any): Record<string, Record<string, boolean>> => {
  // If settingsJson already contains the new nested structure, return it
  if (settingsJson?.permissions && typeof settingsJson.permissions === 'object' && !('productManagement' in settingsJson.permissions)) {
    const parsed: Record<string, Record<string, boolean>> = {};
    MODULES.forEach(mod => {
      parsed[mod.id] = {};
      ACTIONS.forEach(act => {
        parsed[mod.id][act.id] = !!settingsJson.permissions[mod.id]?.[act.id];
      });
    });
    return parsed;
  }

  // Otherwise, if it's the old structure or empty, migrate or use defaults
  const oldPerms = settingsJson?.permissions || {};
  const oldAccess = settingsJson?.accessControl || {};
  const hasOldData = Object.keys(oldPerms).length > 0 || Object.keys(oldAccess).length > 0;

  if (!hasOldData) {
    return JSON.parse(JSON.stringify(DEFAULT_MATRIX_PERMISSIONS));
  }

  // Migrate old data
  const matrix: Record<string, Record<string, boolean>> = {};
  
  const mapOld = (moduleKey: string, oldPermKey: string, oldAccessObj: any) => {
    const isModuleEnabled = !!oldPerms[oldPermKey];
    matrix[moduleKey] = {
      view: isModuleEnabled && !!oldAccessObj.read,
      create: isModuleEnabled && !!oldAccessObj.create,
      edit: isModuleEnabled && !!oldAccessObj.update,
      delete: isModuleEnabled && !!oldAccessObj.delete,
      approve: isModuleEnabled && (!!oldAccessObj.approveProducts || !!oldAccessObj.approveVendors),
      export: isModuleEnabled && !!oldAccessObj.exportReports,
    };
  };

  mapOld('dashboard', 'dashboardAnalytics', oldAccess);
  mapOld('subVendors', 'vendorManagement', oldAccess);
  mapOld('products', 'productManagement', oldAccess);
  mapOld('categories', 'productManagement', oldAccess);
  mapOld('orders', 'orders', oldAccess);
  mapOld('customers', 'customers', oldAccess);
  mapOld('payments', 'orders', oldAccess);
  mapOld('payouts', 'payouts', oldAccess);
  mapOld('coupons', 'coupons', oldAccess);
  mapOld('reports', 'reports', oldAccess);
  mapOld('reviews', 'reviews', oldAccess);
  mapOld('notifications', 'notifications', oldAccess);
  mapOld('analytics', 'dashboardAnalytics', oldAccess);

  return matrix;
};

export default function EditHubWizard() {
  ...
  useEffect(() => {
    if (hubId) fetchHubDetails();
  }, [hubId]);

  const fetchHubDetails = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`${API_URL}/api/admin-ops/hubs/${hubId}`);
      if (!res.ok) throw new Error('Failed to fetch hub details');
      const data = await res.json();

      setFormData({
        hubUsername: data.username || '',
        password: '',
        confirmPassword: '',
        email: data.email || '',
        mobileNumber: data.mobile || '',
        hubId: data.vendorHubId || data.hubCode || '',
      ...
      setMatrixPermissions(parsePermissions(data.settingsJson));
    } catch (err) {
      console.error(err);
      addToast('Error', 'Failed to load hub details');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.hubUsername || !formData.email || !formData.mobileNumber || !formData.name) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/admin-ops/hubs/wizard/${hubId}`, {
        method: 'PUT',
      ...
      const data = await res.json();
      if (res.ok) {
        addToast('Success', 'Hub updated successfully!');
        router.push('/admin/hubs');
      } else {
        addToast('Error', data.error || 'Failed to update Hub');
      }
    } catch (err) {
      addToast('Error', 'Failed to update Hub');
    } finally {
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-500 pb-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Edit Vendor Hub</h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] mt-1">Single Page Edit</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} name="password" placeholder="Leave blank to keep unchanged" value={formData.password} onChange={handleInputChange} className="w-full bg-slate-50 rounded-xl px-4 py-3 pr-12 text-sm font-bold border border-slate-200 outline-none focus:border-emerald-500 transition-colors" />
            ...
            <div>
              ...
              <div className="relative">
                <input type={showConfirmPassword ? "text" : "password"} name="confirmPassword" value={formData.confirmPassword} onChange={handleInputChange} className="w-full bg-slate-50 rounded-xl px-4 py-3 pr-12 text-sm font-bold border border-slate-200 outline-none focus:border-emerald-500 transition-colors" />
