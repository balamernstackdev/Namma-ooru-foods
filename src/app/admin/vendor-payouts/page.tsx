...
export default function AdminVendorPayoutsList() {
  // Filter states
  ...
  const totalPayoutsAmount = payouts?.reduce((sum: number, p: any) => sum + Number(p.payableAmount), 0) || 0;
  const pendingPayoutsAmount = payouts?.filter((p: any) => p.status === 'PENDING').reduce((sum: number, p: any) => sum + Number(p.payableAmount), 0) || 0;
  const settledPayoutsAmount = payouts?.filter((p: any) => p.status === 'SETTLED').reduce((sum: number, p: any) => sum + Number(p.payableAmount), 0) || 0;
  const totalCommissionAmount = payouts?.reduce((sum: number, p: any) => sum + Number(p.commission), 0) || 0;

  // Trigger Weekly Calculation Engine
  const handleCalculatePayouts = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/vendor/payouts/calculate`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) throw new Error(data.error || 'Failed calculation');
      toast.success(data.message || 'Calculation completed successfully!');
      mutatePayouts();
    } catch (error: any) {
      console.error(error);
      toast.error(error.message || 'Error running calculation engine');
    } finally {
  ...
  const handleManualSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/vendor/payouts/manual`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
      ...
      toast.error(err.message || 'Error creating manual entry');
    } finally {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `vendor_payouts_export_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
        <div className="flex flex-col">
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter italic flex items-center gap-3">
            ...
            <span>Vendor Payouts <span className="text-emerald-600">Hub</span></span>
          </h1>
          <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mt-2">
            Automated Weekly Calculation & Settlements Console
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 shrink-0">
          ...
          >
            <FileSpreadsheet size={16} /> Add Manual Entry
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
        <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm flex items-center gap-5 h-full">
          ...
          <div className="flex flex-col min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 truncate">Gross Payable</p>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm flex items-center gap-5 h-full">
          ...
          <div className="flex flex-col min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 truncate">Pending Settled</p>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm flex items-center gap-5 h-full">
          ...
          <div className="flex flex-col min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 truncate">Total Commissions</p>
          ...
          <table className="w-full min-w-[1000px] text-left border-collapse admin-data-table">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/40">
                ...
                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Vendor Store</th>
                <th className="py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Week Range</th>
                  ...
                  <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-5 px-6 text-center">
                      <span className="text-xs font-bold text-slate-700">
                        {p.totalOrders} <span className="text-slate-400">({p.deliveredOrders})</span>
                      </span>
                    </td>
                    ...
                    <td className="py-5 px-6">
                      {p.payoutDate ? (
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-700">{new Date(p.payoutDate).toLocaleDateString()}</span>
                          <span className="text-[9px] font-mono text-slate-400 mt-1 uppercase truncate max-w-[80px]" title={p.transactionRef}>{p.transactionRef}</span>
                        </div>
                      ) : (
                    ...
                    <td className="py-5 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/vendor-payouts/${p.id}`}
              ...
              <div key={p.id} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-50/50 rounded-2xl p-4 border border-slate-100/50 text-xs">
                  <div>
                    ...
                    <span className="font-bold text-slate-800">{p.totalOrders} <span className="text-slate-400">({p.deliveredOrders} Deliv)</span></span>
                  </div>
                  <div>
                    <span className="text-[9px] text-slate-400 font-black uppercase tracking-wider block">Gross Revenue</span>
                ...
                <div className="flex justify-between items-center text-[10px] text-slate-400 px-1">
                  <span>Settled: {p.payoutDate ? new Date(p.payoutDate).toLocaleDateString() : '—'}</span>
                ...
                <div className="flex items-center gap-2 justify-end pt-1">
                  <Link
                    href={`/admin/vendor-payouts/${p.id}`}
                    className="h-11 flex-1 flex items-center justify-center gap-1.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 text-xs font-bold transition-all no-underline"
                  >
      ...
      {isManualModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsManualModalOpen(false)} />
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div>
                <h3 className="text-xl font-black text-emerald-950">Manual Ledger Entry</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Create a custom week-wise payout record</p>
              </div>
              <button onClick={() => setIsManualModalOpen(false)} className="h-10 w-10 rounded-full bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-slate-50 hover:text-slate-900 transition-all">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-8 max-h-[70vh] overflow-y-auto">
              <form onSubmit={handleManualSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Week Start</label>
                    <input
                      type="date"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Week End</label>
                    <input
                      type="date"
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Net Payable Amount (₹) <span className="text-red-500">*</span></label>
                    <input
                      type="number"
                      step="0.01"
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <option value="PENDING">PENDING</option>
                      <option value="ON_HOLD">ON HOLD</option>
                      <option value="SETTLED">SETTLED (Legacy)</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-slate-100 flex justify-end gap-4">
                  <button type="button" onClick={() => setIsManualModalOpen(false)} className="px-6 py-4 rounded-xl text-xs font-bold text-slate-500 hover:bg-slate-50 transition-all">Cancel</button>
                  <button type="submit" disabled={isCreatingManual} className="px-8 py-4 rounded-xl bg-emerald-600 text-white text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all disabled:opacity-50">
                    {isCreatingManual ? 'Saving...' : 'Create Ledger Entry'}
                  </button>
                </div>
              </form>
            </div>
