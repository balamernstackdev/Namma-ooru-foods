...
export default function EditProduct({ id }: { id?: string }) {
   ...
   const { data: subVendorsRes } = useSWR(user ? `${API_URL}/api/vendor-hub/sub-vendors?limit=1000` : null, fetcherAuth);
   ...
   useEffect(() => {
      if (product && !product.error && !isHydrated && categories.length > 0) {
         ...
         const initialBrandId = product.subVendorId?.toString() || product.subVendor?.id?.toString() || '';
      ...
      try {
         ...
         if (res.ok) {
            ...
            if (data && data.success) {
               addToast('Success', data.message || 'Product updated successfully');
      ...
      <div className="flex flex-col items-center justify-center py-40 px-6 bg-[#F8FAF7] min-h-screen">
         <div className="bg-white border border-[#E5E7EB] shadow-xl rounded-[20px] p-10 max-w-md w-full text-center flex flex-col items-center gap-6">
            ...
            <div className="w-full pt-4 border-t border-[#E5E7EB] flex gap-3">
               <button
                  type="button"
                  onClick={() => router.push('/hub/products')}
            ...
            <div className="w-full flex items-center justify-between">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     ...
                     <p className="text-[10px] font-bold text-[#6B7280] uppercase tracking-widest mt-0.5">Modify product details</p>
                  </div>
               </div>
