...
export default function PageDetailLoader() {
  const searchParams = useSearchParams();
  const router = useRouter();
  // Accept ?slug= for new CMS pages or ?id= for legacy
  ...
  if (!slug) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center py-20 px-8 text-center">
        <h1 className="text-4xl font-black text-slate-800 uppercase tracking-tighter">No Page Specified</h1>
        <p className="text-slate-400 font-medium mt-4 max-w-sm mx-auto">The requested page could not be found.</p>
        <button
          onClick={() => router.push('/')}
  ...
  if (error || !post || post.error) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center py-20 px-8 text-center bg-white">
        ...
        <h1 className="text-4xl md:text-5xl font-black text-slate-800 uppercase tracking-tighter">Page Not Found</h1>
