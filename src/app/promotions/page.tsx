  ...
  return (
    <div className={`group relative flex flex-col bg-white rounded-[3.5rem] overflow-hidden transition-all duration-700 h-full ${upcoming ? 'opacity-80 scale-95 border border-dashed border-gray-200' : 'hover:shadow-[0_40px_120px_rgba(0,0,0,0.12)] hover:-translate-y-4 shadow-sm border border-gray-100'}`}>
      ...
      <div className="p-6 md:p-10 flex flex-col flex-1">
        ...
        <p className="text-gray-400 text-sm md:text-base font-medium leading-relaxed mb-6 flex-1">{promo.description}</p>

        <div className="mt-auto pt-6 border-t border-slate-50 flex flex-col gap-4">
          <button
            disabled={upcoming}
            onClick={() => onCopy(promo.code)}
            className={`w-full h-14 md:h-16 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center px-8 transition-all active:scale-95 shadow-premium group/btn ${copiedCode === promo.code
...
export default function PromotionsPage() {
  ...
  return (
    <div className="w-full bg-[#fafafa] min-h-screen">
      ...
      <div className="w-full flex justify-center pt-16 md:pt-24 pb-8 section-no-bottom">
