   ...
   return (
      <div className="flex flex-col bg-[#f8fafc] w-full min-h-screen">
         ...
         <div className="w-full bg-white border-b border-slate-200 sticky top-0 z-[100] shadow-sm shadow-slate-100/50">
            <div className="max-w-[1600px] mx-auto px-4 md:px-6 py-3 flex items-center justify-between text-xs text-slate-500">
               <div className="flex items-center gap-2 font-medium">
                  ...
                  <span className="text-slate-900 font-bold uppercase tracking-wider">Bestsellers</span>
               </div>
               <div className="hidden md:flex items-center gap-4 text-slate-900 font-bold">
                  <div className="flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full text-[10px]">
                     <ShieldCheck size={12} /> 100% Natural Sourcing
                  </div>
               </div>
            </div>
         </div>
               ...
               <main className="flex-1 px-4 md:px-8 py-6 md:py-8 pb-28 lg:pb-8 flex flex-col gap-6 min-h-screen max-w-full overflow-hidden">
                     ...
                     <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <div className="relative flex-1 md:flex-initial min-w-[180px]">
                           <select
                              ...
                              className="w-full h-10 border border-slate-200 bg-white pl-4 pr-10 text-[11px] font-black uppercase tracking-wider text-slate-800 rounded-lg appearance-none cursor-pointer transition-all hover:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                           >
                  ...
                  {(activeCategory !== 'All' || priceFilter !== 'all' || ratingFilter > 0) && (
                     <div className="flex flex-wrap items-center gap-2 py-1">
                        <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 mr-1">Active Filters:</span>
                        {activeCategory !== 'All' && (
                           <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-900 px-3 py-1.5 rounded-full text-[11px] font-bold border border-emerald-100 shadow-sm">
                              ...
                              <button onClick={() => setActiveCategory('All')} className="hover:text-rose-500"><X size={12} /></button>
                           </div>
                        )}
                        {priceFilter !== 'all' && (
                           <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-900 px-3 py-1.5 rounded-full text-[11px] font-bold border border-emerald-100 shadow-sm">
                              Price: {priceFilter === 'under200' ? 'Under ₹200' : priceFilter === '200to500' ? '₹200-₹500' : 'Above ₹500'}
                              <button onClick={() => setPriceFilter('all')} className="hover:text-rose-500"><X size={12} /></button>
                           </div>
                        )}
                        {ratingFilter > 0 && (
                           <div className="flex items-center gap-1.5 bg-emerald-50 text-emerald-900 px-3 py-1.5 rounded-full text-[11px] font-bold border border-emerald-100 shadow-sm">
                              Rating: {ratingFilter}+ Stars
                              <button onClick={() => setRatingFilter(0)} className="hover:text-rose-500"><X size={12} /></button>
                           </div>
                        )}
                        <button onClick={resetFilters} className="text-[11px] font-black text-emerald-600 underline uppercase tracking-wider ml-1 hover:text-emerald-800">Reset</button>
                     </div>
                  )}
                  ...
                  {sortedProducts.length === 0 ? (
                     <div className="bg-white rounded-2xl border border-slate-200 py-24 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-slate-800 font-black uppercase text-[14px] tracking-wider">No Matching Items</h3>
                        <p className="text-slate-400 text-[12px] mt-1 max-w-xs">Try relaxing your active filter combinations to find more products.</p>
                        <button onClick={resetFilters} className="mt-5 px-5 py-2 bg-emerald-900 text-white rounded-lg text-[11px] font-bold uppercase">Show all products</button>
                     </div>
                  ) : (
         ...
         <div className="lg:hidden fixed bottom-24 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 bg-slate-900 text-white px-2.5 py-2 rounded-full shadow-2xl shadow-slate-950/50 border border-slate-700/40 font-black uppercase tracking-widest text-[10px]">
            ...
            <span className="px-3 text-slate-300">{sortedProducts.length} Items</span>
         </div>
               ...
               <>
                  ...
                  >
                     ...
                     <div className="flex items-center justify-between px-6 pb-4 border-b border-slate-100">
                        <span className="text-sm font-black text-slate-900 uppercase tracking-widest">Filters & Sort</span>
                        <button onClick={() => setShowFilters(false)} className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-600">
                           <X size={16} />
                        </button>
                     </div>
                     ...
                     <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-8 no-scrollbar">
                        {/* Sort option inside mobile drawer */}
                        <div className="space-y-3">
                           ...
                           <div className="grid grid-cols-2 gap-2">
                              {(['rating', 'price_asc', 'price_desc', 'newest'] as const).map(opt => (
                                 ...
                                 >
                                    {opt === 'rating' ? 'Top Rated' : opt === 'price_asc' ? 'Price Low-High' : opt === 'price_desc' ? 'Price High-Low' : 'Newest'}
                                 </button>
                              ))}
                           </div>
                        </div>
                     ...
                     <div className="p-6 border-t border-slate-100 flex gap-3 bg-white">
                        <button onClick={resetFilters} className="flex-1 h-12 border border-slate-200 text-slate-700 rounded-xl font-bold text-xs uppercase tracking-wider active:bg-slate-50">
                           Reset All
                        </button>
                        <button onClick={() => setShowFilters(false)} className="flex-1 h-12 bg-emerald-900 text-white rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-emerald-900/20 active:scale-[0.98]">
                           Apply Filters
                        </button>
                     </div>
                  </motion.div>
